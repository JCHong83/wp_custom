...
if(!function_exists('mbw_get_latest_mb_basic')){
	function mbw_get_latest_mb_basic($mode,$data){		
		...
		if($mode!="sidebar" && !empty($title)) 
			$latest_html	.= '<div class="mb-latest-title"><a href="'.get_permalink($post_id).'" title="'.esc_attr($title).'"'.$head_title_style1.'>'.$title.'</a></div>';
			...
			foreach($latest_list as $latest_item){
				...
				$latest_html	.= '<tr><td class="mb-latest-item-title">';				
					if($link_type=="homepage"){
						$latest_html	.= '<a href="'.$latest_item[$board_field["fn_homepage"]].'" title="'.esc_attr($latest_item[$board_field["fn_title"]]).'"'.$link_target.'>';
					}else if($link_type!="false"){
						$latest_html	.= '<a href="'.$latest_permalink.$latest_item[$board_field["fn_pid"]].'" title="'.esc_attr($latest_item[$board_field["fn_title"]]).'"'.$link_target.'>';
					}					
