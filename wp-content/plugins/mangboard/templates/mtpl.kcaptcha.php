	...
	function mbw_get_kcaptcha_template($mode, $data){
		...
		if($item_type=='kcaptcha_img'){
			...
			if($kcaptcha_mode!=1 && (function_exists("imagejpeg") || function_exists("imagepng") || function_exists("imagegif"))){
				if($index==1){
					$template_start	= '<img'.__STYLE("width:80px;height:32px;vertical-align:top;").' onclick="mb_reloadImage()" class="mb_kcaptcha cursor_pointer" src="'.$kcaptcha_url.'" id="'.$item_id.'" class="border-ccc-1"'.$t_alt.'/>';				
				}else{
					$template_start	= '<img'.__STYLE("width:80px;height:32px;vertical-align:top;").' onclick="mb_reloadImage_class()" class="mb_kcaptcha cursor_pointer" src="'.$kcaptcha_url.'" id="'.$item_id.$index.'" class="border-ccc-1"'.$t_alt.'/>';				
				}		
				...
				$template_start	.= '<input'.$data["ext"].__STYLE("width:".$data["width"].";height:".$data["height"]." !important;border:1px solid #ccc;vertical-align:middle;margin:0 !important;".$data["style"]).' maxlength="6" value="'.$keystring.'" title="'.$keystring.'"  readonly />';
			}
			$template_start	.= '<input'.$data["ext"].__STYLE("width:".$data["width"].";height:".$data["height"]." !important;".$data["style"]).' name="'.mbw_set_form_name($data["type"]).'" maxlength="6" value="'.$data["value"].'" type="text"'.$t_title.' />';
		}		
