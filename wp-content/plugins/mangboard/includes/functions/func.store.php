...
if(!function_exists('mbw_init_board_panel')){
	function mbw_init_board_panel(){
		global $mstore;
		$mstore->init_board_panel();
	}	
}
...
if(!function_exists('mbw_get_vars')){
	function mbw_get_vars($key){
		global $mb_vars;
		if(isset($mb_vars[$key])) return $mb_vars[$key];
		else return "";
	}	
}
...
if(!function_exists('mbw_set_board_languages')){
	function mbw_set_board_languages($lang=""){
		global $mstore;
		$mstore->set_board_languages($lang);
	}	
}
...
if(!function_exists('mbw_get_timestamp')){
	function mbw_get_timestamp(){
		if(function_exists('current_datetime')){
			if(mbw_get_vars("current_timestamp")==""){
				$datetime		= current_datetime();
				$temp_array	= get_object_vars($datetime);
				if(!empty($temp_array['date'])){
					$timestamp	= strtotime($temp_array['date']);
				}else{
					$timestamp	= current_time('timestamp');
				}				
				mbw_set_vars("current_timestamp",$timestamp);
			}else{
				$timestamp	= mbw_get_vars("current_timestamp");
			}
		}else{
			$timestamp	= current_time('timestamp');
		}
...
if(!function_exists('mbw_set_item_index')){
	function mbw_set_item_index($value){
		global $mstore;
		return $mstore->set_item_index($value);
	}	
}
if(!function_exists('mbw_get_model_key')){
	function mbw_get_model_key(){
		global $mstore;
		return $mstore->get_model_key();
	}	
}
if(!function_exists('mbw_set_cookie')){
	function mbw_set_cookie($name,$value,$expire=0,$path="/",$domain="",$secure=false,$httponly=true){
		global $mstore;
		if(mbw_get_option("ssl_mode") || strpos(MBW_HOME_URL,'https')===0) $secure		= is_ssl();
		$mstore->set_cookie($name,$value);
		if(version_compare(PHP_VERSION, '7.3.0', '<')){
			if($secure && function_exists('mbw_init_commerce_panel') && mbw_get_option("use_card_pay")==1){
				$path	.= "; SameSite=None ";
			}
			@setcookie($name, $value, $expire, $path, $domain, $secure, $httponly);
		}else{
			$cookie_options		= array("expires"=>$expire,"path"=>$path,"domain"=>$domain,"secure"=>$secure,"httponly"=>$httponly);
			if($secure && function_exists('mbw_init_commerce_panel') && mbw_get_option("use_card_pay")==1){
...
if(!function_exists('mbw_init_board_order')){
	function mbw_init_board_order(){
		global $mstore;
		$mstore->init_board_order();
	}	
}
