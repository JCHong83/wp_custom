...
Class MStore
{
	...
	public function check_get_param(){
		if(is_array($this->params)){
			$preg_exception		= array("search_text","search_add_text1","search_add_text2","search_add_text3","redirect_to","category1","category2","category3","se_text1","se_text2","se_text3","se_text4","se_text5");
	...
	public function check_post_param(){
		if(is_array($this->params)){
			$check_param		= array("board_name","board_pid","board_page","mode","board_action","order_by","order_type","search_field","user_pid","parent_pid","parent_user_pid","page_size");
			foreach($check_param  as $key){
				if(!empty($this->params[$key])){
					$this->params[$key]			= mbw_value_filter($this->params[$key]);
				}
			}
			$date_param		= array("calendar_date","reg_date","modify_date","search_date");
